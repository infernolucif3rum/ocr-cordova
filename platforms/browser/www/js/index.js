...
var app = {
    ...
    takePicture: function () {
        navigator.camera.getPicture(function (picUri) {
            ...
            document.getElementById('temp-status').innerHTML = picUri.replace("file://", "");
            console.log(picUri);
            path = picUri.replace("file://", "");
            alert("Image Loaded!");
        }, function (message) {
    ...
    recogPicture: function () {
        ...
        if (photoURI !== null) {
            path = photoURI;
            TesseractPlugin.recognizeText(path, 'eng', function (recognizedText) {
                console.log(recognizedText);
                document.getElementById('temp-status').innerHTML = "success";
                alert(recognizedText);
                if (recognizedText.match(/([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9._-]+)/gi)){
                    alert("Found EMAIL:::: "+ recognizedText.match(/([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9._-]+)/gi));
    ...
    sendMail: function () {
        ...
        if (document.getElementById('temp-status2').innerHTML != "TEMP"){
            alert("Found email!"+ document.getElementById('temp-status2').innerHTML);
        } else {
            alert("No mail found :(");
        }
    }
